...
const BottomNav: React.FC<{ view: ViewType; setView: (v: ViewType) => void }> = ({ view, setView }) => {
    ...
    const items: { id: ViewType; icon: string; label: string }[] = [
        ...
        { id: 'spots', icon: 'fa-heart', label: '收藏' }, // 口袋名單改到這
        ...
        <div className="space-y-6 pb-12">
            ...
            <div className="bg-white p-5 rounded-3xl shadow-sm border border-milk-tea-100 space-y-4">
                <h3 className="font-black mb-1 text-sm text-milk-tea-800 uppercase tracking-tighter flex items-center gap-2">
                    <i className="fa-solid fa-cloud-arrow-up"></i> 同步與備份
                </h3>
                <div className="space-y-2">
                    <button onClick={() => {
                        ...
                        alert("同步碼已複製！");
                    }} className="w-full py-3 bg-milk-tea-800 text-white rounded-xl text-xs font-black active:scale-95 transition-transform">
                        複製同步碼 (分享給隊友)
                    </button>
                    <div className="flex gap-2">
                        <input value={syncCode} onChange={e => setSyncCode(e.target.value)} placeholder="貼上同步碼" className="flex-1 p-3 bg-milk-tea-50 rounded-xl text-xs outline-none font-black border border-milk-tea-100" />
                        <button onClick={() => {
                            if (!syncCode) return;
                            try {
                                const decoded = JSON.parse(decodeURIComponent(atob(syncCode)));
                                if (confirm("這會覆蓋目前資料，確定嗎？")) { setData(decoded); alert("同步成功！"); }
                            } catch(e) { alert("代碼無效。"); }
                        }} className="px-4 bg-milk-tea-100 text-milk-tea-800 rounded-xl text-xs font-black active:scale-95">導入</button>
                    </div>
                </div>
                <div className="grid grid-cols-2 gap-2 pt-2">
                    <button onClick={handleExportJSON} className="py-3 bg-white border border-milk-tea-300 text-milk-tea-800 rounded-xl text-xs font-black">導出 JSON</button>
                    <button onClick={() => fileInputRef.current?.click()} className="py-3 bg-white border border-milk-tea-300 text-milk-tea-800 rounded-xl text-xs font-black">導入 JSON</button>
                    <input type="file" ref={fileInputRef} onChange={handleImportJSON} accept=".json" className="hidden" />
                </div>
            </div>

            <div className="bg-red-50 p-5 rounded-3xl shadow-sm border border-red-100">
                <button onClick={handleClearAll} className="w-full py-4 bg-white border-2 border-red-200 text-red-500 rounded-2xl text-xs font-black">
                    清除所有資料 (重置)
                </button>
            </div>
        </div>
